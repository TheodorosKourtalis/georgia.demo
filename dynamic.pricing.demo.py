...
def calculate_price(product, current_dt):
    """
    Computes the current price of a product based on the current time.
    The degradation is linear from 7:00 (start_price) today to 5:00 (end_price) tomorrow.
    If current time is before 7:00, returns start_price;
    if after 5:00 (next day), returns end_price.
    """
    # Greek time zone
    tz = pytz.timezone("Europe/Athens")
    # Determine today's date in Greek time
    today = current_dt.astimezone(tz).date()
    start_time = tz.localize(datetime.datetime.combine(today, datetime.time(7, 0)))
    # End time is at 5:00 AM the next day
    end_time = tz.localize(datetime.datetime.combine(today + datetime.timedelta(days=1), datetime.time(5, 0)))
...
if page == "Demo":
    ...
    st.write("Prices degrade linearly from 7:00 AM (today) to 5:00 AM (tomorrow) for all users.")
    st.markdown("---")
    ...
    st.markdown("---")
    st.markdown("### Price Schedule from 7:00 AM to 5:00 AM (next day)")
    today = now.date()
    start_dt = tz.localize(datetime.datetime.combine(today, datetime.time(7, 0)))
    end_dt = tz.localize(datetime.datetime.combine(today + datetime.timedelta(days=1), datetime.time(5, 0)))
